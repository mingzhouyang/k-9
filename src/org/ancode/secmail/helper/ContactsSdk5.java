 ...
 */
public class ContactsSdk5 extends org.ancode.secmail.helper.Contacts {
	/**
	 * The order in which the search results are returned by
	 * {@link #searchContacts(CharSequence)}.
	 */
	protected static final String SORT_ORDER = Email.TIMES_CONTACTED + " DESC, " + Contacts.DISPLAY_NAME + ", "
			+ Email._ID;
	...
	public void createContact(final Address email) {
		...
		contactIntent.putExtra(Intents.EXTRA_CREATE_DESCRIPTION, email.toString());
	...
	public Cursor searchContacts(final CharSequence constraint) {
		...
		final Cursor c = mContentResolver.query(uri, PROJECTION, null, null, SORT_ORDER);
	...
	public ContactItem extractInfoFromContactPickerIntent(final Intent data) {
		...
		try {
			...
			cursor = mContentResolver.query(Email.CONTENT_URI, PROJECTION, Email.CONTACT_ID + "=?",
					new String[] { id }, null);
