...
public class ImapStore extends Store {
	...
	public static ImapStoreSettings decodeUri(String uri) {
		...
		return new ImapStoreSettings(host, port, connectionSecurity, authenticationType, username, password,
	...
	public static String createUri(ServerSettings server) {
		...
		try {
			...
			passwordEnc = (server.password != null) ? URLEncoder.encode(server.password, "UTF-8") : "";
		...
		try {
			...
			throw new IllegalArgumentException("Invalid authentication type: " + server.authenticationType);
		}
	...
	public static class ImapStoreSettings extends ServerSettings {
		...
		protected ImapStoreSettings(String host, int port, ConnectionSecurity connectionSecurity,
				String authenticationType, String username, String password, boolean autodetectNamespace,
				String pathPrefix) {
			super(STORE_TYPE, host, port, connectionSecurity, authenticationType, username, password);
		...
		public ServerSettings newPassword(String newPassword) {
			return new ImapStoreSettings(host, port, connectionSecurity, authenticationType, username, newPassword,
					autoDetectNamespace, pathPrefix);
		}
	}
	...
	private List<? extends Folder> listFolders(ImapConnection connection, boolean LSUB) throws IOException,
		...
		List<ImapResponse> responses = connection.executeSimpleCommand(String.format("%s \"\" %s", commandResponse,
	 ...
	 * {@link #listFolders(org.ancode.secmail.mail.store.ImapStore.ImapConnection, boolean)}
	...
	private void autoconfigureFolders(final ImapConnection connection) throws IOException, MessagingException {
		...
		final List<ImapResponse> responses = connection.executeSimpleCommand(String.format("%s%s \"\" %s",
				commandResponse, commandOptions, encodeString(getCombinedPrefix() + "*")));
			...
			if (ImapResponseParser.equalsIgnoreCase(response.get(0), commandResponse)) {
				...
				for (int i = 0, count = attributes.size(); i < count; i++) {
					...
					if (attribute.equals("\\Drafts")) {
						...
						if (K9.DEBUG)
							Log.d(K9.LOG_TAG, "Folder auto-configuration detected draft folder: " + decodedFolderName);
					} else if (attribute.equals("\\Sent")) {
						mAccount.setSentFolderName(decodedFolderName);
						if (K9.DEBUG)
							Log.d(K9.LOG_TAG, "Folder auto-configuration detected sent folder: " + decodedFolderName);
					} else if (attribute.equals("\\Spam")) {
						mAccount.setSpamFolderName(decodedFolderName);
						if (K9.DEBUG)
							Log.d(K9.LOG_TAG, "Folder auto-configuration detected spam folder: " + decodedFolderName);
					} else if (attribute.equals("\\Trash")) {
						mAccount.setTrashFolderName(decodedFolderName);
						if (K9.DEBUG)
							Log.d(K9.LOG_TAG, "Folder auto-configuration detected trash folder: " + decodedFolderName);
					}
				}
			}
		}
...
/**
	...
	class ImapFolder extends Folder {
		...
		public Map<String, String> copyMessages(Message[] messages, Folder folder) throws MessagingException {
			...
			try {
				...
				mConnection.sendCommand(String.format("UID COPY %s %s", Utility.combine(uids, ','), remoteDestName),
						false);
				...
				if (response.size() > 1) {
					...
					if (responseList instanceof ImapList) {
						...
						if (copyList.size() >= 4 && copyList.getString(0).equals("COPYUID")) {
							List<String> srcUids = ImapUtility.getImapSequenceValues(copyList.getString(2));
							List<String> destUids = ImapUtility.getImapSequenceValues(copyList.getString(3));
								...
								if (srcUids.size() == destUids.size()) {
									...
									if (K9.DEBUG) {
										Log.v(K9.LOG_TAG, "Parse error: size of source UIDs "
												+ "list is not the same as size of destination " + "UIDs list.");
									}
								}
							} else {
								if (K9.DEBUG) {
		...
		public void delete(Message[] messages, String trashFolderName) throws MessagingException {
			...
			if (trashFolderName == null || getName().equalsIgnoreCase(trashFolderName)) {
				...
				if (!exists(remoteTrashName)) {
					 ...
					 */
					if (K9.DEBUG)
						Log.i(K9.LOG_TAG, "IMAPMessage.delete: attempting to create remote '" + trashFolderName
								+ "' folder for " + getLogId());
				...
				if (exists(remoteTrashName)) {
					if (K9.DEBUG)
						Log.d(K9.LOG_TAG, "IMAPMessage.delete: copying remote " + messages.length + " messages to '"
		...
		protected Message[] getMessages(final int start, final int end, Date earliestDate,
				final boolean includeDeleted, final MessageRetrievalListener listener) throws MessagingException {
			...
			ImapSearcher searcher = new ImapSearcher() {
				public List<ImapResponse> search() throws IOException, MessagingException {
					return executeSimpleCommand(String.format("UID SEARCH %d:%d%s%s", start, end, dateSearchString,
		...
		protected Message[] getMessages(final List<Long> mesgSeqs, final boolean includeDeleted,
			...
			ImapSearcher searcher = new ImapSearcher() {
				public List<ImapResponse> search() throws IOException, MessagingException {
					return executeSimpleCommand(String.format("UID SEARCH %s%s",
							Utility.combine(mesgSeqs.toArray(), ','), includeDeleted ? "" : " NOT DELETED"));
				}
			};
			return search(searcher, listener);
		}
			...
			ImapSearcher searcher = new ImapSearcher() {
				public List<ImapResponse> search() throws IOException, MessagingException {
					return executeSimpleCommand(String.format("UID SEARCH UID %s%s",
							Utility.combine(mesgUids.toArray(), ','), includeDeleted ? "" : " NOT DELETED"));
				}
			};
			return search(searcher, listener);
		}
				...
				try {
					...
					do {
						...
						if (response.mTag == null && ImapResponseParser.equalsIgnoreCase(response.get(1), "FETCH")) {
							...
							if (message == null) {
								if (K9.DEBUG)
									Log.d(K9.LOG_TAG, "Do not have message in messageMap for UID " + uid + " for "
											+ getLogId());
		...
		@Override
		public void fetchPart(Message message, Part part, MessageRetrievalListener listener) throws MessagingException {
			checkOpen();
			...
			try {
				mConnection.sendCommand(String.format("UID FETCH %s (UID %s)", message.getUid(), fetch), false);
		...
		protected void handleUntaggedResponse(ImapResponse response) {
			if (response.mTag == null && response.size() > 1) {
				...
				if (ImapResponseParser.equalsIgnoreCase(response.get(1), "EXPUNGE") && mMessageCount > 0) {
					mMessageCount--;
					if (K9.DEBUG)
						Log.d(K9.LOG_TAG, "Got untagged EXPUNGE with mMessageCount " + mMessageCount + " for "
								+ getLogId());
				}
				// if (response.size() > 1) {
				// Object bracketedObj = response.get(1);
				...
				if (bodyDisposition != null && !bodyDisposition.isEmpty()) {
					...
					if ((bodyDisposition.size() > 1) && (bodyDisposition.get(1) instanceof ImapList)) {
						...
						for (int i = 0, count = bodyDispositionParams.size(); i < count; i += 2) {
							contentDisposition.append(String.format(";\n %s=\"%s\"", bodyDispositionParams.getString(i)
									.toLowerCase(Locale.US), bodyDispositionParams.getString(i + 1)));
						}
					}
				}
			...
			try {
				...
				for (Message message : messages) {
					mConnection.sendCommand(String.format("APPEND %s (%s) {%d}",
							encodeString(encodeFolderName(getPrefixedName())), combineFlags(message.getFlags()),
		...
		public String getUidFromMessageId(Message message) throws MessagingException {
			try {
				...
				List<ImapResponse> responses = executeSimpleCommand(String.format("UID SEARCH HEADER MESSAGE-ID %s",
		...
		public void setFlags(Message[] messages, Flag[] flags, boolean value) throws MessagingException {
			...
			try {
				executeSimpleCommand(String.format("UID STORE %s %sFLAGS.SILENT (%s)", Utility.combine(uids, ','),
	...
	public static class ImapConnection {
		...
		public void open() throws IOException, MessagingException {
			...
			try {
				...
				try {
					if (mSettings.getAuthType() == AuthType.CRAM_MD5) {
						...
						receiveCapabilities(executeSimpleCommand(
								String.format("LOGIN %s %s", ImapStore.encodeString(mSettings.getUsername()),
				...
				if (K9.DEBUG)
					Log.d(K9.LOG_TAG, "NAMESPACE = " + hasCapability(CAPABILITY_NAMESPACE) + ", mPathPrefix = "
							+ mSettings.getPathPrefix());
							...
							if (ImapResponseParser.equalsIgnoreCase(response.get(0), COMMAND_NAMESPACE)) {
								...
								if (personalNamespaces != null && personalNamespaces instanceof ImapList) {
									...
									if (firstNamespace != null && firstNamespace instanceof ImapList) {
										...
										if (K9.DEBUG)
											Log.d(K9.LOG_TAG, "Got path '" + mSettings.getPathPrefix()
													+ "' and separator '" + mSettings.getPathDelimeter() + "'");
									}
								}
							}
						}
					...
					try {
						...
						for (ImapResponse response : nameResponses) {
							if (ImapResponseParser.equalsIgnoreCase(response.get(0), "LIST")) {
								...
								mSettings.setCombinedPrefix(null);
								if (K9.DEBUG)
									Log.d(K9.LOG_TAG, "Got path delimeter '" + mSettings.getPathDelimeter() + "' for "
											+ getLogId());
							}
						}
					} catch (Exception e) {
		...
		protected void authCramMD5() throws AuthenticationFailedException, MessagingException {
			try {
				...
				byte[] b64CRAM = Authentication.computeCramMd5Bytes(mSettings.getUsername(), mSettings.getPassword(),
						b64NonceTrim);
		...
		public List<ImapResponse> executeSimpleCommand(String command, boolean sensitive,
			...
			do {
				...
				if (response.mTag != null && !response.mTag.equalsIgnoreCase(tag)) {
					Log.w(K9.LOG_TAG, "After sending tag " + tag + ", got tag response from previous command "
	...
	public class ImapFolderPusher extends ImapFolder implements UntaggedHandler {
						...
						try {
							...
							if (newUidNext == -1) {
								...
								if (highestUid != -1L) {
									...
									if (K9.DEBUG)
										Log.d(K9.LOG_TAG, "highest UID = " + highestUid + ", set newUidNext to "
												+ newUidNext);
								}
							}
							...
							if (newUidNext > startUid) {
								...
								if (K9.DEBUG)
									Log.i(K9.LOG_TAG, "Needs sync from uid " + startUid + " to " + newUidNext + " for "
											+ getLogId());
							...
							if (stop.get()) {
								...
								if (idleFailureCount.incrementAndGet() > IDLE_FAILURE_COUNT_LIMIT) {
									...
									receiver.pushError(
											"Push disabled for " + getName() + " after " + idleFailureCount.get()
		...
		protected int processUntaggedResponse(long oldMessageCount, ImapResponse response, List<Long> flagSyncMsgSeqs,
					...
					if (ImapResponseParser.equalsIgnoreCase(responseType, "EXPUNGE")) {
						...
						for (long msgSeqNum : msgSeqs) {
							...
							if (msgSeqNum == msgSeq) {
								...
								if (K9.DEBUG) {
									Log.d(K9.LOG_TAG, "Scheduling removal of UID " + uid + " because msgSeq "
											+ msgSeqNum + " was expunged");
								}
								...
								String uid = msgSeqUidMap.get(msgSeqNum);
								if (K9.DEBUG) {
									Log.d(K9.LOG_TAG, "Reducing msgSeq for UID " + uid + " from " + msgSeqNum + " to "
											+ (msgSeqNum - 1));
								}
								msgSeqUidMap.remove(msgSeqNum);
		...
		public void stop() {
			...
			if (conn != null) {
				...
				Log.w(K9.LOG_TAG, "Attempt to interrupt null mConnection to stop pushing on folderPusher for "
						+ getLogId());
			}
		}
		...
		public void handleAsyncUntaggedResponse(ImapResponse response) {
			...
			if (stop.get()) {
				if (K9.DEBUG)
					Log.d(K9.LOG_TAG, "Got async untagged response: " + response + ", but stop is set for "
							+ getLogId());
				...
				if (response.mTag == null) {
					if (response.size() > 1) {
						...
						if (ImapResponseParser.equalsIgnoreCase(responseType, "EXISTS")
							...
							if (K9.DEBUG)
								Log.d(K9.LOG_TAG, "Got useful async untagged response: " + response + " for "
										+ getLogId());
	...
	private static class FetchBodyCallback implements ImapResponseParser.IImapResponseCallback {
		...
		@Override
		public Object foundLiteral(ImapResponse response, FixedLengthInputStream literal) throws IOException, Exception {
			if (response.mTag == null && ImapResponseParser.equalsIgnoreCase(response.get(1), "FETCH")) {
				ImapList fetchList = (ImapList) response.getKeyedValue("FETCH");
				String uid = fetchList.getKeyedString("UID");
