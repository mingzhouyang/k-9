 ...
 */
public class MessageList extends K9Activity implements OnClickListener, AdapterView.OnItemClickListener,
		AnimationListener {
	...
	static {
		...
		final Map<SortType, Comparator<MessageInfoHolder>> map = new EnumMap<SortType, Comparator<MessageInfoHolder>>(
				SortType.class);
	...
	/**
	 * Remember the selection to be consistent between menu display and menu
	 * item selection
	 */
	...
	class MessageListHandler {
			...
			runOnUiThread(new Runnable() {
				...
				public void run() {
					for (MessageInfoHolder message : messages) {
						if (message != null
								&& (mFolderName == null || (message.folder != null && message.folder.name
										.equals(mFolderName)))) {
		...
		private void setWindowTitle() {
			...
			if (mFolderName != null) {
				...
				if (mTitle != null) {
					String dispString = mAdapter.mListener.formatHeader(MessageList.this, mTitle, mUnreadMessageCount,
							getTimeFormat());
					setTitle(dispString);
				} else {
	...
	/**
	 * Show the message list that was used to open the {@link MessageView} for a
	 * message.
	 * 
	 * <p>
	 * <strong>Note:</strong> The {@link MessageList} instance should still be
	 * around and all we do is bring it back to the front (see the activity
	 * flags).<br>
	 * Out of sheer paranoia we also set the extras that were used to create the
	 * original {@code MessageList} instance. Using those, the activity can be
	 * recreated in the unlikely case of it having been killed by the OS.
	 * </p>
	 * 
	 * @param context
	 *            The {@link Context} instance to invoke the
	 *            {@link Context#startActivity(Intent)} method on.
	 * @param extras
	 *            The extras used to create the original {@code MessageList}
	 *            instance.
	 * 
	...
	public static void actionHandle(Context context, String title, String queryString, boolean integrate, Flag[] flags,
			Flag[] forbiddenFlags) {
		Intent intent = new Intent(context, MessageList.class);
	...
	/**
	 * Creates and returns an intent that opens Unified Inbox or All Messages
	 * screen.
	 */
	...
	private void initializeMessageList(Intent intent, boolean create) {
		boolean returnFromMessageView = intent.getBooleanExtra(EXTRA_RETURN_FROM_MESSAGE_VIEW, false);

		if (!create && returnFromMessageView) {
			// We're returning from the MessageView activity with
			...
			new Thread() {
				...
				public void run() {
					...
					if (mFolderName != null) {
						...
						mController.searchLocalMessagesSynchronous(mAccountUuids, mFolderNames, null, mQueryString,
								mIntegrate, mQueryFlags, mForbiddenFlags, mAdapter.mListener);
					}
	...
	private void onOpenMessage(MessageInfoHolder message) {
		...
		if (cryptUuidMap != null && !cryptUuidMap.isEmpty() && !message.message.getFolder().getAccount().hasReg()) {
			// crypt message
	...
	public Dialog onCreateDialog(int id) {
		...
		case DIALOG_MARK_ALL_AS_READ:
			return ConfirmationDialog.create(this, id, R.string.mark_all_as_read_dlg_title,
					getString(R.string.mark_all_as_read_dlg_instructions_fmt, mCurrentFolder.displayName),
		...
		case R.id.dialog_confirm_spam:
			return ConfirmationDialog.create(this, id, R.string.dialog_confirm_spam_title, "" /*
																							 ...
																							 * below
																							 */,
					R.string.dialog_confirm_spam_confirm_button, R.string.dialog_confirm_spam_cancel_button,
					new Runnable() {
						@Override
						public void run() {
						...
						@Override
						public void run() {
							// event for cancel, we don't need this reference
							// any more
							mActiveMessages = null;
						}
					});
		...
		case DIALOG_REG_CONFIRM: {
			return ConfirmationDialog.create(this, id, R.string.apply_reg_encrypt,
					getString(R.string.reg_encrypt_confirm_dialog), R.string.okay_action, R.string.cancel_action,
					new Runnable() {
						@Override
						public void run() {
							onRegCrypt();
	...
	public boolean onPrepareOptionsMenu(Menu menu) {
		...
		if (mQueryString != null) {
			...
			if (!mController.isMoveCapable(mAccount)) {
				...
				if (mCurrentFolder != null && !mAccount.getInboxFolderName().equals(mCurrentFolder.name)) {
					menu.findItem(R.id.check_mail).setVisible(false);
				}
				menu.findItem(R.id.batch_archive_op).setVisible(false);
				menu.findItem(R.id.batch_spam_op).setVisible(false);
	...
	public boolean onContextItemSelected(MenuItem item) {
		...
		final MessageInfoHolder holder = mSelectedMessage == null ? (MessageInfoHolder) mAdapter.getItem(info.position)
				: mSelectedMessage;
		...
		case R.id.same_sender: {
			MessageList
					.actionHandle(MessageList.this, "From " + holder.sender, holder.senderAddress, false, null, null);
			break;
		}
		}
	 ...
	 * @param selected
	 *            true if this was an attempt to select (i.e. left to right).
	 */
	private void handleSwipe(final MotionEvent downMotion, final boolean selected) {
		int[] listPosition = new int[2];
		mListView.getLocationOnScreen(listPosition);
		int position = mListView.pointToPosition((int) downMotion.getRawX() - listPosition[0],
				(int) downMotion.getRawY() - listPosition[1]);
	...
	class MessageListAdapter extends BaseAdapter {
				...
				if (message.isSet(Flag.DELETED)) {
					...
					if (m == null) {
						if (updateForMe(account, folderName)) {
							...
							messageHelper.populate(m, message, new FolderInfoHolder(MessageList.this, messageFolder,
		...
		public Object getItem(int position) {
			try {
				...
				Log.e(K9.LOG_TAG,
						"getItem(" + position + "), but folder.messages.size() = " + mAdapter.messages.size(), e);
			}
			return null;
		}
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (message != null) {
				...
				if (holder.preview != null) {
					...
					str.setSpan(new AbsoluteSizeSpan(mFontSizes.getMessageListSender(), true), 0, noSender.length(),
							Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
				} else {
		...
		private void bindView(final int position, final View view, final MessageViewHolder holder,
			...
			if (holder.preview != null) {
				...
				holder.preview.setText(new SpannableStringBuilder(recipientSigil(message)).append(message.sender)
						.append(" ").append(message.message.getPreview()), TextView.BufferType.SPANNABLE);
				Spannable str = (Spannable) holder.preview.getText();
				...
				str.setSpan(new StyleSpan(senderTypeface), 0, message.sender.length() + 1,
						Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
				str.setSpan(new AbsoluteSizeSpan(mFontSizes.getMessageListSender(), true), 0,
						message.sender.length() + 1, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);

				// set span for preview message.
				str.setSpan(new ForegroundColorSpan(Color.rgb(128, 128, 128)), // How
																				// do
																				// I
																				// can
																				// specify
			...
			if (holder.encrypt_status != null) {
				if (message.message.isEncry()) {
					...
					holder.encrypt_status.setBackgroundDrawable(getResources().getDrawable(
							R.drawable.ic_button_lock_min));
			...
			holder.subject.setCompoundDrawablesWithIntrinsicBounds(message.answered ? mAnsweredIcon : null, // left
					// message.message.isEncrypt() ? mLockIcon : mUnlockIcon, //
					// top
					null, message.message.hasRealAttachments() ? mAttachmentIcon : null, // right
					null); // bottom
	...
	public void onClick(View v) {
		...
		if (!messageList.isEmpty()) {
			if (v == mBatchDeleteButton) {
				...
				mController.setFlag(messageList.toArray(EMPTY_MESSAGE_ARRAY), (v == mBatchReadButton ? Flag.SEEN
						: Flag.FLAGGED), newState);
			}
		} else {
			// Should not happen
	...
	/**
	 * Helper method to get a List of message ready to be processed. This
	 * implementation will return a list containing the sole argument.
	 * 
	 * @param holder
	 *            Never {@code null}.
	...
	/**
	 * Helper method to get a List of message ready to be processed. This
	 * implementation will iterate over messages and choose the checked ones.
	 * 
	 * @return Never {@code null}.
	 */
	 ...
	 *            Never {@code null}.
	 */
	private void copyOrMove(final List<MessageInfoHolder> holders, final String destination,
	 ...
	 * 
	 * @param prefs
	 *            A {@link Preferences} instance that might be used to retrieve
	...
	private Account getCurrentAccount(Preferences prefs) {
		Account account = null;
		if (mQueryString != null && !mIntegrate && mAccountUuids != null && mAccountUuids.length == 1) {
