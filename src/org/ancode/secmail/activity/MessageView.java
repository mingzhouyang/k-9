...
public class MessageView extends K9Activity implements OnClickListener {
	...
	/**
	 * The extras used to create the {@link MessageList} instance that created
	 * this activity. May be {@code null}.
	 * 
	...
	public void onCreate(Bundle icicle) {
		...
		mMessageView.setAttachmentCallback(new AttachmentFileDownloadCallback() {
			...
			public void showFileBrowser(final AttachmentView caller) {
				FileBrowserHelper.getInstance().showFileBrowserActivity(MessageView.this, null,
						MessageView.ACTIVITY_CHOOSE_DIRECTORY, callback);
		...
		if (icicle != null) {
			// TODO This code seems unnecessary since the icicle should already
			// be thawed in onRestoreInstanceState().
	...
	private void setupDisplayMessageButtons() {
		...
		// If moving isn't support at all, then all of them must be disabled
		// anyway.
		if (mController.isMoveCapable(mAccount)) {
			// Only enable the button if they have an archive folder and it's
			// not the current folder.
			mArchive.setEnabled(!mMessageReference.folderName.equals(mAccount.getArchiveFolderName())
					&& mAccount.hasArchiveFolder());
			// Only enable the button if the Spam folder is not the current
			// folder and not NONE.
	...
	private void onFlag() {
		if (mMessage != null) {
			...
			mController.setFlag(mAccount, mMessage.getFolder().getName(), new Message[] { mMessage }, Flag.FLAGGED,
					newState);
	...
	/**
	 * Handle a right-to-left swipe starting at the edge of the screen as
	...
	private void onToggleRead() {
		if (mMessage != null) {
			mController.setFlag(mAccount, mMessage.getFolder().getName(), new Message[] { mMessage }, Flag.SEEN,
	...
	// TODO: when switching to API version 8, override onCreateDialog(int,
	// Bundle)
	...
	protected Dialog onCreateDialog(final int id) {
		...
		case R.id.dialog_confirm_delete:
			return ConfirmationDialog.create(this, id, R.string.dialog_confirm_delete_title,
					R.string.dialog_confirm_delete_message, R.string.dialog_confirm_delete_confirm_button,
					R.string.dialog_confirm_delete_cancel_button, new Runnable() {
						@Override
						public void run() {
							delete();
						}
		...
		case R.id.dialog_confirm_spam:
			return ConfirmationDialog.create(this, id, R.string.dialog_confirm_spam_title, getResources()
					.getQuantityString(R.plurals.dialog_confirm_spam_message, 1),
	...
	public boolean onPrepareOptionsMenu(Menu menu) {
		if (menu != null) {
			...
			if (additionalHeadersItem != null) {
				additionalHeadersItem
						.setTitle(mMessageView.additionalHeadersVisible() ? R.string.hide_full_header_action
								: R.string.show_full_header_action);
			}

			if (mMessage != null) {
				int actionTitle = mMessage.isSet(Flag.SEEN) ? R.string.mark_as_unread_action
	...
	class Listener extends MessagingListener {
		...
		public void loadMessageForViewHeadersAvailable(final Account account, String folder, String uid,
			...
			runOnUiThread(new Runnable() {
				public void run() {
					if (!clonedMessage.isSet(Flag.X_DOWNLOADED_FULL) && !clonedMessage.isSet(Flag.X_DOWNLOADED_PARTIAL)) {
		...
		@Override
		public void loadMessageForViewBodyAvailable(final Account account, String folder, String uid,
				final Message message) {
			if (!mMessageReference.uid.equals(uid) || !mMessageReference.folderName.equals(folder)
					|| !mMessageReference.accountUuid.equals(account.getUuid())) {
				return;
			...
			runOnUiThread(new Runnable() {
				...
				public void run() {
					try {
						mMessage = message;
						mMessageView.setMessage(account, (LocalMessage) message, mPgpData, mController, mListener);
