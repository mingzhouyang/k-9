...
public class AccountSetupIncoming extends K9Activity implements OnClickListener {
	...
	private static final ConnectionSecurity[] CONNECTION_SECURITY_TYPES = { ConnectionSecurity.NONE,
			ConnectionSecurity.SSL_TLS_OPTIONAL, ConnectionSecurity.SSL_TLS_REQUIRED,
			ConnectionSecurity.STARTTLS_OPTIONAL, ConnectionSecurity.STARTTLS_REQUIRED };
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		SpinnerOption authTypeSpinnerOptions[] = { new SpinnerOption(0, AUTH_TYPES[0]),
	...
	private void validateFields() {
		mNextButton.setEnabled(Utility.requiredFieldValid(mUsernameView) && Utility.requiredFieldValid(mPasswordView)
		...
		if (resultCode == RESULT_OK) {
			if (Intent.ACTION_EDIT.equals(getIntent().getAction())) {
				...
				try {
					...
					URI uri = new URI(oldUri.getScheme(), usernameEnc + ":" + passwordEnc, oldUri.getHost(),
	...
	protected void onNext() {
		try {
			ConnectionSecurity connectionSecurity = CONNECTION_SECURITY_TYPES[(Integer) ((SpinnerOption) mSecurityTypeView
			...
			ServerSettings settings = new ServerSettings(mStoreType, host, port, connectionSecurity, authType,
