...
public class AccountSetupCheckSettings extends K9Activity implements OnClickListener {
	...
	public static void actionCheckSettings(Activity context, Account account, boolean checkIncoming,
		...
		new Thread() {
			...
			public void run() {
				...
				try {
					...
					showErrorDialog(R.string.account_setup_failed_dlg_auth_message_fmt, afe.getMessage() == null ? ""
							: afe.getMessage());
					...
					showErrorDialog(R.string.account_setup_failed_dlg_server_message_fmt, (t.getMessage() == null ? ""
							: t.getMessage()));
		...
		mHandler.post(new Runnable() {
			public void run() {
				...
				new AlertDialog.Builder(AccountSetupCheckSettings.this)
						...
						.setCancelable(true)
						.setNegativeButton(getString(R.string.account_setup_failed_dlg_continue_action),
							...
							for (List<?> subjectAlternativeName : subjectAlternativeNames) {
								...
								if (name.equalsIgnoreCase(storeURIHost) || name.equalsIgnoreCase(transportURIHost)) {
									...
									altNamesText.append("Subject(alt): ").append(name).append(",...\n");
										...
										&& (storeURIHost.endsWith(name.substring(2)) || transportURIHost.endsWith(name
												.substring(2)))) {
									// TODO: localize this string
									altNamesText.append("Subject(alt): ").append(name).append(",...\n");
								}
						...
						.setPositiveButton(getString(R.string.account_setup_failed_dlg_invalid_certificate_accept),
								new DialogInterface.OnClickListener() {
									public void onClick(DialogInterface dialog, int which) {
										...
										AccountSetupCheckSettings.actionCheckSettings(AccountSetupCheckSettings.this,
