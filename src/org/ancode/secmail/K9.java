...
public class K9 extends Application {
	...
	public static class Intents {
		...
		public static class Share {
			 ...
			 * ("com.fsck.k9.intent.extra.FROM") because of different semantics
			 * (String array vs. string with comma separated email addresses)
			 */
	...
	private static void setServicesEnabled(Context context, boolean enabled, Integer wakeLockId) {
		...
		if (!enabled
				&& pm.getComponentEnabledSetting(new ComponentName(context, MailService.class)) == PackageManager.COMPONENT_ENABLED_STATE_ENABLED) {
			/*
		...
		for (Class<?> clazz : classes) {
			...
			boolean alreadyEnabled = pm.getComponentEnabledSetting(new ComponentName(context, clazz)) == PackageManager.COMPONENT_ENABLED_STATE_ENABLED;
		...
		MessagingController.getInstance(this).addListener(new MessagingListener() {
			private void broadcastIntent(String action, Account account, String folder, Message message) {
				try {
					...
					if (K9.DEBUG)
						Log.d(K9.LOG_TAG, "Broadcasted: action=" + action + " account=" + account.getDescription()
								+ " folder=" + folder + " message uid=" + message.getUid());

				} catch (MessagingException e) {
					Log.w(K9.LOG_TAG, "Error: action=" + action + " account=" + account.getDescription() + " folder="
							+ folder + " message uid=" + message.getUid());
				}
			}
	...
	public static void loadPrefs(Preferences prefs) {
		...
		if (notificationHideSubject == null) {
			...
			// the new enum.
			sNotificationHideSubject = (sprefs.getBoolean("keyguardPrivacy", false)) ? NotificationHideSubject.WHEN_LOCKED
		...
		mAttachmentDefaultPath = sprefs.getString("attachmentdefaultpath", Environment.getExternalStorageDirectory()
				.toString());
	...
	private void maybeSetupStrictMode() {
		...
		try {
			...
			// Discard , as it means we're not running on a device with strict
			// mode
	...
	public static Method getMethod(Class<?> classObject, String methodName) {
		try {
			...
			Log.e(K9.LOG_TAG,
					"Error while using reflection to get method " + classObject.toString() + "." + methodName, e);
		}
		return null;
	}
