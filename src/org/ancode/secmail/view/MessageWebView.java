...
public class MessageWebView extends TitleBarWebView {
	...
	/**
	 * We use WebSettings.getBlockNetworkLoads() to prevent the WebView that
	 * displays email bodies from loading external resources over the network.
	 * Unfortunately this method isn't exposed via the official Android API.
	 * That's why we use reflection to be able to call the method.
	 */
	...
	public void setText(String text, String contentType) {
		String content = text;
		if (K9.getK9Theme() == K9.THEME_DARK) {
			// It's a little wrong to just throw in the <style> before the
			// opening <html>
			...
			content = "<style>* { background: black ! important; color: white !important }"
					...
					+ ":visited, :visited * { color: #551A8B !important }</style> " + content;
		}
